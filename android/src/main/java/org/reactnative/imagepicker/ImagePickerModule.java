...
import static org.reactnative.imagepicker.Utils.*;
...
public class ImagePickerModule extends ReactContextBaseJavaModule implements ActivityEventListener {
    ...
    public ImagePickerModule(ReactApplicationContext reactContext) {
        ...
        this.barcodeDetector = new RNBarcodeDetector(reactContext);
    }
    ...
    void onAssetsObtained(final List<Uri> fileUris) {
        try {
            if (!options.detectBarcode) {
                callback.invoke(getResponseMap(fileUris, options, reactContext));
                callback = null;
                return;
            }
            if (fileUris.isEmpty()) {
                callback.invoke(getErrorMap(errOthers, "empty selected"));
                callback = null;
                return;
            }
